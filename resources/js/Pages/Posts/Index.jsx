...
const FeaturedToggle = ({ post, featuredCount, onToggle, loading }) => {
    ...
    return (
        ...
        >
            <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-primary shadow transition-transform ${
    ...
    return (
        <AdminLayout title="Posts">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
                    <div className="relative w-full sm:w-64">
                        ...
                        <input
                            ...
                            className="block w-full rounded-xl border border-primary/20 bg-primary/5 py-2 pl-10 pr-3 text-sm text-primary placeholder-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-colors"
                        />
                    </div>
                    <Link
                        href="/admin/posts/create"
                        className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 px-5 py-2.5 text-sm font-semibold text-primary shadow-lg shadow-indigo-500/25 transition-all hover:shadow-xl hover:shadow-indigo-500/30 hover:brightness-110 active:scale-[0.98]"
                    >
            ...
            <div className="mb-6 flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                <div className="flex gap-1 sm:gap-1.5">
                    {[0, 1, 2, 3, 4, 5, 6, 7].map(i => (
                        <div
                            key={i}
                            className={`h-2 w-6 rounded-full transition-colors sm:w-12 ${
                                i < currentFeaturedCount ? 'bg-amber-400' : 'bg-primary/10'
                            }`}
                        />
                    ))}
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-slate-300">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {posts.data?.map((post) => (
                                <tr key={post.id} className="transition-colors hover:bg-primary/[0.03]">
                                    ...
                                    <td className="px-4 py-3">
                                        {post.image ? (
                                            ...
                                            <div className="flex h-12 w-20 items-center justify-center rounded-lg bg-primary/5 text-slate-600">
            ...
            {(!posts.data || posts.data.length === 0) && (
                <div className="mt-4 flex flex-col items-center justify-center rounded-2xl border border-dashed border-primary/20 py-16">
                    <div className="mb-4 text-4xl">🎬</div>
                    <h3 className="text-lg font-semibold text-primary">No posts yet</h3>
