    ...
    return (
        <AdminLayout title="Create Role">
            ...
            <div className="mx-auto max-w-4xl rounded-2xl border border-white/5 bg-white/[0.03] p-8 backdrop-blur-sm">
                <h2 className="mb-1 text-2xl font-bold text-white">Create New Role</h2>
                ...
                <form onSubmit={submit}>
                    <div className="mb-8">
                        ...
                        <input type="text" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="e.g., manager" className="w-full max-w-md rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-slate-500 outline-none focus:border-indigo-500/50 focus:ring-2 ring-indigo-500/50" />
                    ...
                    <div className="flex gap-3 border-t border-white/5 pt-6">
                        <button type="submit" disabled={processing} className="rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 hover:brightness-110 active:scale-[0.98] disabled:opacity-50">{processing ? 'Creating...' : 'Create Role'}</button>
