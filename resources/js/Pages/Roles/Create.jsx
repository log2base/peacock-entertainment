...
export default function RolesCreate({ permissions, groupedPermissions }) {
    ...
    return (
        <AdminLayout title="Create Role">
            <Link href="/admin/roles" className="mb-6 inline-flex items-center gap-1 text-sm text-slate-400 hover:text-primary">← Back to Roles</Link>
            <div className="mx-auto max-w-4xl rounded-2xl border border-primary/10 bg-primary/[0.03] p-8 backdrop-blur-sm">
                <h2 className="mb-1 text-2xl font-bold text-primary">Create New Role</h2>
                <p className="mb-8 text-sm text-slate-400">Define a new role and assign permissions</p>
                <form onSubmit={submit}>
                    <div className="mb-8">
                        <label className="mb-2 block text-sm font-medium text-slate-300">Role Name <span className="text-red-400">*</span></label>
                        <input type="text" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="e.g., manager" className="w-full max-w-md rounded-xl border border-primary/20 bg-primary/5 px-4 py-2.5 text-sm text-primary placeholder-slate-500 outline-none focus:border-indigo-500/50 focus:ring-2 ring-indigo-500/50" />
                        {errors.name && <p className="mt-2 text-sm text-red-400">{errors.name}</p>}
                    </div>
                    <div className="mb-8">
                        <div className="mb-4 flex items-center justify-between">
                            <label className="text-sm font-medium text-slate-300">Permissions</label>
                            <button type="button" onClick={toggleAll} className="rounded-lg bg-primary/5 px-3 py-1.5 text-xs font-medium text-indigo-300 hover:bg-primary/10">{data.permissions.length === permissions.length ? 'Deselect All' : 'Select All'}</button>
                        </div>
                        <div className="space-y-6">
                            {Object.entries(groupedPermissions).map(([group, perms]) => {
                                ...
                                return (
                                    <div key={group} className="rounded-xl border border-primary/10 bg-primary/[0.02] p-5">
                                        <div className="mb-3 flex items-center gap-3">
                                            <button type="button" onClick={() => toggleGroup(perms)} className={`flex h-5 w-5 items-center justify-center rounded-md border transition-all ${allSel ? 'border-indigo-500 bg-indigo-500 text-primary' : someSel ? 'border-indigo-500/50 bg-indigo-500/30 text-primary' : 'border-primary/30'}`}>
                    ...
                    <div className="flex gap-3 border-t border-primary/10 pt-6">
                        <button type="submit" disabled={processing} className="rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-2.5 text-sm font-semibold text-primary shadow-lg shadow-indigo-500/25 hover:brightness-110 active:scale-[0.98] disabled:opacity-50">{processing ? 'Creating...' : 'Create Role'}</button>
                        <Link href="/admin/roles" className="rounded-xl bg-primary/5 px-6 py-2.5 text-sm font-medium text-slate-400 hover:bg-primary/10 hover:text-primary">Cancel</Link>
                    </div>
                </form>
            </div>
        </AdminLayout>
