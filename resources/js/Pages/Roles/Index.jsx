        ...
        <AdminLayout title="Roles">
            ...
            <form onSubmit={handleSearch} className="mb-6">
                <div className="relative max-w-md">
                    <input
                        ...
                        className="w-full rounded-xl border border-white/10 bg-white/5 py-2.5 pl-10 pr-4 text-sm text-white placeholder-slate-500 outline-none ring-indigo-500/50 transition-all focus:border-indigo-500/50 focus:bg-white/[0.07] focus:ring-2 backdrop-blur-sm"
                    />
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {roles.data?.map((role) => (
                    <div
                        key={role.id}
                        className="group relative overflow-hidden rounded-2xl border border-white/5 bg-white/[0.03] p-6 backdrop-blur-sm transition-all duration-300 hover:border-indigo-500/20 hover:bg-white/[0.06] hover:shadow-xl hover:shadow-indigo-500/5"
                    >
                        {/* Glow effect */}
                        ...
                        <div className="relative">
                            ...
                            <div className="mb-5 flex flex-wrap gap-1.5">
                                {role.permissions?.slice(0, 4).map((perm) => (
                                    <span
                                        key={perm.id}
                                        className="rounded-lg bg-white/5 px-2 py-1 text-[11px] text-slate-400"
                                    >
                                        {perm.name}
                                    </span>
                                ))}
                                {(role.permissions?.length || 0) > 4 && (
                                    <span className="rounded-lg bg-white/5 px-2 py-1 text-[11px] text-slate-500">
