    ...
    return (
        <AdminLayout title="User Roles">
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Link href="/admin/users/create" className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 px-5 py-2.5 text-sm font-semibold text-primary shadow-lg shadow-indigo-500/25 hover:brightness-110 active:scale-[0.98]">
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row">
                <form onSubmit={handleSearch} className="relative flex-1 max-w-md">
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users..." className="w-full rounded-xl border border-primary/20 bg-primary/5 py-2.5 pl-10 pr-4 text-sm text-primary placeholder-slate-500 outline-none focus:border-indigo-500/50 focus:ring-2 ring-indigo-500/50 backdrop-blur-sm" />
                ...
                <select value={roleFilter} onChange={(e) => handleRoleFilter(e.target.value)} className="rounded-xl border border-primary/20 bg-primary/5 px-4 py-2.5 text-sm text-primary outline-none focus:border-indigo-500/50 focus:ring-2 ring-indigo-500/50 backdrop-blur-sm">
            ...
            {/* Users Table */}
            <div className="overflow-hidden rounded-2xl border border-primary/10 bg-primary/[0.03] backdrop-blur-sm">
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-white/5">
                        {users.data?.map((user) => (
                            <tr key={user.id} className="group transition-colors hover:bg-primary/[0.02]">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-xs font-bold text-primary">{user.name?.charAt(0)}</div>
                                        <div>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex flex-wrap gap-1.5">
                                        {user.permissions?.length > 0 ? user.permissions.slice(0, 3).map((p) => (
                                            <span key={p.id} className="rounded-lg bg-primary/5 px-2 py-0.5 text-[11px] text-slate-400">{p.name}</span>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                        <Link href={`/admin/users/${user.id}/roles/edit`} className="rounded-lg bg-primary/5 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-primary/10 hover:text-primary">Manage</Link>
                ...
                {(!users.data || users.data.length === 0) && (
                    <div className="flex flex-col items-center py-16">
                        ...
                        <h3 className="text-lg font-semibold text-primary">No users found</h3>
                    </div>
                )}
            </div>
            ...
            {users.links && users.last_page > 1 && (
                <div className="mt-8 flex items-center justify-center gap-1">
                    {users.links.map((link, i) => (
                        <Link key={i} href={link.url || '#'} className={`rounded-lg px-3 py-1.5 text-sm font-medium ${link.active ? 'bg-indigo-500/20 text-indigo-300' : link.url ? 'text-slate-400 hover:bg-primary/5' : 'cursor-not-allowed text-slate-600'}`} dangerouslySetInnerHTML={{ __html: link.label }} preserveState />
                    ))}
                </div>
            )}
        </AdminLayout>
