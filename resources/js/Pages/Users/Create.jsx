...
export default function UserCreate({ roles }) {
    ...
    return (
        <AdminLayout title="Create User">
            <Link href="/admin/users/roles" className="mb-6 inline-flex items-center gap-1 text-sm text-slate-400 hover:text-primary">← Back to Users</Link>
            <div className="mx-auto max-w-2xl rounded-2xl border border-primary/10 bg-primary/[0.03] p-8 backdrop-blur-sm">
                <h2 className="mb-1 text-2xl font-bold text-primary">Create New User</h2>
                ...
                <form onSubmit={submit}>
                    ...
                    <div className="mb-8">
                        ...
                        <input type="password" value={data.password} onChange={(e) => setData('password', e.target.value)} required placeholder="••••••••" className="w-full rounded-xl border border-primary/20 bg-primary/5 px-4 py-2.5 text-sm text-primary placeholder-slate-500 outline-none focus:border-indigo-500/50 focus:ring-2 ring-indigo-500/50" />
                    ...
                    <div className="mb-8">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            {roles.map((role) => (
                                <label key={role.id} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition-all ${data.roles.includes(role.name) ? 'border-indigo-500/30 bg-indigo-500/10' : 'border-primary/10 bg-primary/[0.02] hover:border-primary/20 hover:bg-primary/[0.04]'}`}>
                                    <div className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-md border transition-all ${data.roles.includes(role.name) ? 'border-indigo-500 bg-indigo-500' : 'border-primary/30'}`}>
                                        {data.roles.includes(role.name) && <svg className="h-2.5 w-2.5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                                    </div>
                    ...
                    <div className="flex gap-3 border-t border-primary/10 pt-6">
                        <button type="submit" disabled={processing} className="rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-2.5 text-sm font-semibold text-primary shadow-lg shadow-indigo-500/25 hover:brightness-110 active:scale-[0.98] disabled:opacity-50">{processing ? 'Creating...' : 'Create User'}</button>
