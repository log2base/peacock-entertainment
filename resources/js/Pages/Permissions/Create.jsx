    ...
    return (
        <AdminLayout title="Create Permission">
            ...
            <div className="mx-auto max-w-2xl rounded-2xl border border-primary/10 bg-primary/[0.03] p-8 backdrop-blur-sm">
                <h2 className="mb-1 text-2xl font-bold text-primary">Create New Permission</h2>
                <p className="mb-8 text-sm text-slate-400">Use dot notation like <code className="rounded bg-primary/10 px-1.5 py-0.5 text-indigo-300">module.action</code></p>
                <form onSubmit={submit}>
                    <div className="mb-6">
                        <label className="mb-2 block text-sm font-medium text-slate-300">Permission Name <span className="text-red-400">*</span></label>
                        <input type="text" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="e.g., user.create" className="w-full rounded-xl border border-primary/20 bg-primary/5 px-4 py-2.5 text-sm text-primary placeholder-slate-500 outline-none focus:border-indigo-500/50 focus:ring-2 ring-indigo-500/50" />
                    ...
                    <div className="flex gap-3 border-t border-primary/10 pt-6">
                        <button type="submit" disabled={processing} className="rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 px-6 py-2.5 text-sm font-semibold text-primary shadow-lg shadow-indigo-500/25 hover:brightness-110 active:scale-[0.98] disabled:opacity-50">{processing ? 'Creating...' : 'Create Permission'}</button>
